...
describe("不成功退款的众筹合约(众筹不成功)", function () {
...
describe("测试众筹合约的时限方法", async function () {
    ...
    Crowdsale.withdrawTokens(purchaser, '验证众筹结束后提款错误',true,/RefundablePostDeliveryCrowdsale: goal not reached/);
    Crowdsale.goalReached(false, '验证没有到达众筹目标');//
    Crowdsale.buyTokens(purchaser, EthValue, '结束后购买代币错误',true,/TimedCrowdsale: not open/);
    Crowdsale.claimRefund(purchaser, '众筹结束未到目标退款');

    it('验证purchaser的ETH余额增加' + EthValue, async function () {
        let _purchaserBalance = Math.ceil(web3.utils.fromWei((await web3.eth.getBalance(purchaser) - purchaserBalance).toString(), 'ether'));
        assert.equal(EthValue, _purchaserBalance);
    });
});
