...
describe("测试不成功退款众筹合约的特殊方法(众筹成功)", function () {
    ...
    it('重新测试购买代币方法: buyTokens()', async function () {
        ...
        assert.doesNotReject(CrowdsaleInstance.buyTokens(beneficiary, { value: ether('20'), from: beneficiary }));
    });
    //重新测试众筹是否开始,已开始
    ...
    it('重新测试众筹收入: weiRaised()', async function () {
        assert.equal(ether('20').toString(), (await CrowdsaleInstance.weiRaised()).toString());
    });
    //测试众筹是否关闭,未关闭
    it('众筹未关闭: hasClosed()', async function () {
        assert.ok(!await CrowdsaleInstance.hasClosed());
    ...
    it('购买账户被锁定的余额: balanceOf()', async function () {
        assert.equal(ether((20 * rate).toString()).toString(), (await CrowdsaleInstance.balanceOf(beneficiary)).toString());
    });
    //测试众筹目标
    ...
    it('众筹成功后可以提款: withdrawTokens()', async function () {
        await assert.doesNotReject(CrowdsaleInstance.withdrawTokens(beneficiary));
    });
    //测试众筹成功后不能退回ETH
    it('众筹成功后不能退回ETH: claimRefund()', async function () {
        await assert.rejects(CrowdsaleInstance.claimRefund(beneficiary), /goal reached/);
    });
});
